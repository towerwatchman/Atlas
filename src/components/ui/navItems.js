...
export function getNavItems({
  onToggleGameList, onCheckDbUpdates, onBrowseCatalog, onOpenWishlist,
  onToggleSearchSidebar, onOpenAbout, onOpenCollections,
  ...
  // get-nsfw-status). Defaults to the static flag alone so any caller
  // that hasn't been updated to pass this yet keeps the old behavior
  // instead of silently losing the Browse item.
  browseModeAvailable = BROWSE_MODE_ENABLED,
}) {
  return [
    {
    ...
    } : null,
    {
      ...
      viewBox: '0 0 24 24',
      // NOTE: the prop is called onOpenWishlist for historical reasons but this
      // is the FAVOURITES button — App.jsx passes openFavorites to it. Wiring
      // the actual wishlist here would silently replace favourites.
      onClick: () => {
