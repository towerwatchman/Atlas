...
function parseVDF(text) {
  ...
  for (let line of lines) {
    if (line === '{') {
      ...
      const parts = line.match(/["](.*?)["]\s*["](.*?)["]/) || line.match(/["](.*?)["]/);
      if (parts) {
        if (parts.length === 3) {
          const key = parts[1];
          const value = parts[2];
...
async function getSteamLibraryFolders() {
  ...
  if (fs.existsSync(steamPath)) {
    ...
    if (fs.existsSync(vdfPath)) {
      ...
      console.log('Parsed libraryfolders.vdf:', JSON.stringify(parsed, null, 2));
      for (let key in parsed.libraryfolders) {
        if (!isNaN(parseInt(key))) {
          const lib = parsed.libraryfolders[key];
          if (lib.path) {
            const libPath = path.join(lib.path.replace(/\\\\/g, '\\'), 'steamapps');
    ...
    try {
      ...
      for (const file of files) {
        if (file.startsWith('appmanifest_') && file.endsWith('.acf')) {
          ...
          console.log(`Parsed .acf content for ${file}:`, JSON.stringify(parsed, null, 2));
          ...
          if (appState && appState.appid && appState.name && appState.installdir) {
            ...
            console.log(`Skipping invalid .acf file ${file}: missing required fields`);
          }
        }
      }
    } catch (err) {
