...
//
// ── WHY THIS FILE IS IN workers/ ─────────────────────────────────────────────
//
// It is required by megaHashcashWorker.js, and a worker thread in a packaged
// build can only require files that survive OUTSIDE app.asar. `workers/**/*` is
// in build.asarUnpack; `electron/**/*` is not. While this module lived under
// electron/downloads/hosts/, the worker's require resolved to a path under
// app.asar.unpacked/electron/ that does not exist, and every packaged sign-in
// that MEGA challenged failed before hashing a single byte.
//
// Co-locating it with its only worker consumer means both files are unpacked
// together and the require is a sibling lookup that cannot straddle the asar
// boundary. electron/downloads/hosts/megaHashcash.js re-exports this so the
// main-process side and the tests keep their existing import path.
//
// Anything the worker needs must live in this directory. See
// scripts/check-worker-packaging.js, which enforces that.
//
// ── THE PROTOCOL ─────────────────────────────────────────────────────────────
//
...
/**
 * Mean nonces needed at a given easiness. The search is memoryless -- each hash
 * is an independent 1-in-N draw -- so this is the mean of a geometric
 * distribution, NOT a deadline. Roughly a third of attempts take longer than the
 * mean, which is why a budget set to the expected time fails often.
 */
function expectedAttempts(easiness) {
  return 2 ** 32 / thresholdFromEasiness(easiness);
}

/**
 * Time one hash of the real 12MB message, for the Settings self-test.
 *
 * Measured rather than assumed because the spread across machines is enormous:
 * a CPU with the SHA-NI extension runs this several times faster than one
 * without, which is the difference between a sign-in that takes seconds and one
